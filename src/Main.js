...
const Main = ({ date, movie }) => {
    ...
    const [mvDetailModal, setMovieDetailModal] = useState(false);
    ...
    return (
        <div className="flex">
                ...
                <div className="min-w-15 h-full grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-3 lg:grid-cols-4 xl:gap-x-4">
                    {movie.map((movie) => (
                        <div key={movie.id} className="group relative">
                            <div className="w-full min-h-80 bg-gray-200 aspect-w-1 aspect-h-1 rounded-md overflow-hidden group-hover:opacity-75 lg:h-80 lg:aspect-none cursor-pointer" onClick={() => setMovieDetailModal(true)}>
                                <img
                                    src={movie.imageSrc}
            ...
            { mvDetailModal ? <MovieDetail /> : <MovieDetail />}
        </div>
    );
};
